...
import com.expensereimbursementsystem.beans.User;
import com.expensereimbursementsystem.service.AuthenticationService;
...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		Employee_Table table = authService.authenticateUser(creds);
		if (table != null) {
			// they're real 
			// set user information as session attributes (not request attributes)
			session.setAttribute("employee_id", table.getEmployee_id());
			session.setAttribute("firstName", table.getFirstName());
			session.setAttribute("lastName", table.getLastName());
			session.setAttribute("isAdmin", table.getIsAdmin());
			session.setAttribute("manager_id", table.getManager_id());
